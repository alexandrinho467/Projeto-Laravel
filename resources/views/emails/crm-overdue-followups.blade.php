...
<head>
  ...
  <title>Tarefas atrasadas no CRM</title>
  <style>
    ...
    .hero { background: #635BFF; padding: 24px 40px; text-align: center; }
...
<div class="wrapper">
  ...
  <div class="hero">
    <p class="hero-title">
      {{ $activities->count() === 1 ? '1 tarefa atrasada' : $activities->count() . ' tarefas atrasadas' }}
    </p>
    <p class="hero-sub">Follow-ups do CRM que passaram do prazo.</p>
  </div>

  <div class="body">
    <p class="greeting">
      Olá, <strong>{{ explode(' ', $recipient->name)[0] }}</strong>. As tarefas abaixo estão com o prazo vencido e ainda não foram concluídas.
    </p>

    @foreach($activities as $activity)
    <div class="item-row">
      <div class="item-contact">{{ $activity->contact?->name ?? 'Contato removido' }}</div>
      <div class="item-desc">{{ \Illuminate\Support\Str::limit($activity->description, 120) }}</div>
      <div class="item-due">Prazo: {{ $activity->due_date->format('d/m/Y') }}</div>
      @if($activity->contact)
        <a href="{{ route('admin.crm.contacts.show', $activity->contact) }}" class="item-link">Ver contato →</a>
      @endif
    </div>
    @endforeach
  </div>
